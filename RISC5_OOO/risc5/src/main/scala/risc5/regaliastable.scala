...
//Adding these signals as they are needed in issue queue
  ...
  //val ldst_queue_valid_table = Vec.tabulate(64) { i => UInt(1) }
   ...
   //Adding new registers here
    ...
    val RenameDestTagIssue = Vec.fill(4) { Reg(init = UInt(0, width = 10)) }
    val RenameDestTagLoadStore = Vec.fill(4) { Reg(init = UInt(0, width = 10)) }
 ...
 when (reset) {
  ...
  //Updating the tag for destination registers which is different from the issue queue tag
    ...
    RenameQueueSelect(0) := io.DecodeQueueSelect0 
    RenameQueueSelect(1) := io.DecodeQueueSelect1 
    RenameQueueSelect(2) := io.DecodeQueueSelect2 
    RenameQueueSelect(3) := io.DecodeQueueSelect3 
    RenameStoreSelect(0) := io.DecodeStoreSelect0 
    RenameStoreSelect(1) := io.DecodeStoreSelect1 
    RenameStoreSelect(2) := io.DecodeStoreSelect2 
    RenameStoreSelect(3) := io.DecodeStoreSelect3 
